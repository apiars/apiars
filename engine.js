...
const CLUSTER_CONFIG = Object.freeze({
    includeCodes: [400, 422], // OPTIONAL: Add other 4xx codes if relevant for your API (404, 409, 415, etc.)
    excludeSuccessCodes: true, // Exclude success responses (2xx) from clusters
    maxClustersToShow: 40, // Maximum number of clusters to display in summary output
    minOccurrencesToShow: 1, // Minimum occurrences required to show a cluster
    detailedCurlOutput: true,       // true = curl for each example, false = one curl (old behavior)
    maxCurlsPerCluster: 3,          // Maximum curl per cluster (from 1 to MAX_OCCURRENCES_SAMPLES)
    showMutationDetails: true       // Show mutation details (path, type)
});
const CURL_CONFIG = Object.freeze({
    enabled: true,        // Enable/disable curl generation for bug clusters
    maxBodyLength: 5000   // Maximum body length in curl (bytes). Larger bodies will be truncated or saved to file
});

const STAGE_CONFIG = Object.freeze({
    baseline: Object.freeze({
        enabled: true,
        repeatCount: 3  // Number of identical requests to verify stability, RECOMMENDED: >= 3 for reliable inconsistency detection
    }),
    ...
    fuzzing: Object.freeze({
        ...
        maxObjectsToFuzz: 20,    // Maximum objects to fuzz (0 = no limit, all objects)
        maxArraysToFuzz: 20,     // Maximum arrays to fuzz (0 = no limit, all arrays)
        maxMutationsPerParam: 10 // Maximum mutations per query parameter (0 = no limit)
    }),
    ...
    progressiveDelete: Object.freeze({
        ...
        logMinimalPayload: true,         // Log minimal payload to console (enabled if finalSize < threshold)
        minimalPayloadSizeThreshold: 1000  // Log payload only if < N bytes (0 = always log)
    })
});
...
const ADVANCED_CONFIG = Object.freeze({
    ...
    failFast: Object.freeze({
        enabled: true,
        windowSize: 20,             // Size of sliding window for error detection (last N requests)
        maxServiceErrors: 10,       // Threshold: 5xx errors in last windowSize requests
        maxRateLimitErrors: 5       // Threshold: 429 errors in last windowSize requests
    }),
...
// ============================================================================
// VALIDATION: Ensure failFast.windowSize is valid
// ============================================================================
if (ADVANCED_CONFIG.failFast.enabled) {
    if (ADVANCED_CONFIG.failFast.windowSize < 1) {
        throw new Error('ADVANCED_CONFIG.failFast.windowSize must be >= 1');
    }
    
    // Optional: Warn if windowSize is very large (may miss rapid error spikes)
    if (ADVANCED_CONFIG.failFast.windowSize > 100) {
        console.warn(`⚠️  Warning: failFast.windowSize=${ADVANCED_CONFIG.failFast.windowSize} is very large, may miss rapid error spikes`);
    }
}

// ============================================================================
// VALIDATION: Ensure repeatCount is valid
// ============================================================================
if (STAGE_CONFIG.baseline.repeatCount < 1) {
    throw new Error('STAGE_CONFIG.baseline.repeatCount must be >= 1');
}

// Optional: Warn if repeatCount is unusually high (may slow down testing)
if (STAGE_CONFIG.baseline.repeatCount > 10) {
    console.warn(`⚠️  Warning: baseline.repeatCount=${STAGE_CONFIG.baseline.repeatCount} is high, may slow down testing`);
}

// ============================================================================
// VALIDATION: Ensure fuzzing limits are valid
// ============================================================================
if (STAGE_CONFIG.fuzzing.enabled) {
    if (STAGE_CONFIG.fuzzing.maxObjectsToFuzz < 0 || 
        STAGE_CONFIG.fuzzing.maxArraysToFuzz < 0 || 
        STAGE_CONFIG.fuzzing.maxMutationsPerParam < 0) {
        throw new Error('Fuzzing limits must be >= 0 (0 = no limit)');
    }
    
    // Warn if limits disabled (may be slow for large payloads)
    if (STAGE_CONFIG.fuzzing.maxObjectsToFuzz === 0 && STAGE_CONFIG.fuzzing.fuzzObjects) {
        console.warn('⚠️  Warning: maxObjectsToFuzz=0 (no limit) may be slow for large payloads');
    }
    if (STAGE_CONFIG.fuzzing.maxArraysToFuzz === 0 && STAGE_CONFIG.fuzzing.fuzzArrays) {
        console.warn('⚠️  Warning: maxArraysToFuzz=0 (no limit) may be slow for large payloads');
    }
    if (STAGE_CONFIG.fuzzing.maxMutationsPerParam === 0) {
        console.warn('⚠️  Warning: maxMutationsPerParam=0 (no limit) may generate many requests');
    }
}

// ============================================================================
// VALIDATION: Ensure progressiveDelete.minimalPayloadSizeThreshold is valid
// ============================================================================
if (STAGE_CONFIG.progressiveDelete.enabled) {
    if (STAGE_CONFIG.progressiveDelete.minimalPayloadSizeThreshold < 0) {
        throw new Error('STAGE_CONFIG.progressiveDelete.minimalPayloadSizeThreshold must be >= 0 (0 = always log)');
    }
    
    // Optional: Warn if threshold is very large
    if (STAGE_CONFIG.progressiveDelete.minimalPayloadSizeThreshold > 10000) {
        console.warn(`⚠️  Warning: minimalPayloadSizeThreshold=${STAGE_CONFIG.progressiveDelete.minimalPayloadSizeThreshold} is very large, may suppress useful logs`);
    }
}
...
const PathFilter = {
    ...
    logFilterStats: function(totalNodes, filteredNodes, config) {
        ...
        console.info(`[PathFilter] Total nodes collected: ${totalNodes}`);
        console.info(`[PathFilter] After filtering: ${filteredNodes}`);
        console.info(`[PathFilter] Excluded: ${difference} nodes`);
        
        if (config.debug.verbosity === 'high' && targetPaths.length > 0) {
            console.info(`[PathFilter] Target paths (${targetPaths.length}):`);
            targetPaths.slice(0, 10).forEach(p => console.info(`  - ${p}`));
            if (targetPaths.length > 10) {
                console.info(`  ... and ${targetPaths.length - 10} more`);
            }
        }
...
if (ENABLE_MUTATION_LOGGING) {
        console.info(`✅ Config v${ENGINE_VERSION} - Configuration assembled successfully`);
    }
if (!ENGINE_ENABLED) {
    if (SHOW_BANNER) {
console.info(`╔════════════════════════════════════════════════════════════════════════════
...
const safeAddResult = function(collection, result, maxSize) {
    ...
    if (collection.length >= maxSize) {
        collection.shift();
        if (collection.length === maxSize) {
            console.info(`⚠️ Results array reached limit (${maxSize}), oldest results being evicted`);
        }
    }
...
const ErrorHandler = {
    executeStage: async function(stageName, stageFunc, context, activeConfig) {
        ...
        try {
            ...
            const duration = getTzSafeTimestamp() - startTime;
            console.error(`❌ STAGE FAILED: ${stageName}`);
    ...
    generateEmergencyReport: function(context, stageName, error, activeConfig) {
        console.info('\n');
        console.info('╔════════════════════════════════════════════════════════╗');
        console.info('║       🚨 EMERGENCY REPORT (Partial Results)          ║');
        console.info('╚════════════════════════════════════════════════════════╝');
        ...
        console.info(`Total requests sent: ${stats.total || 0}`);
        console.info(`Success: ${stats.success || 0}`);
        console.info(`Errors: ${stats.errors || 0}`);
        console.info(`Bugs found: ${stats.bugs || 0}`);
        if (context.bugClusters && context.bugClusters.clusters.size > 0) {
            console.info(`Unique bug patterns: ${context.bugClusters.clusters.size}`);
        }
        console.info('─'.repeat(60));
...
// ============================================================================
const buildCurl = function(requestSnapshot) {
    if (!requestSnapshot || !CURL_CONFIG.enabled) {
        return null;
    }
    
    const { method, url, headers, body, bodyFormat } = requestSnapshot;
    ...
    if (headers && typeof headers === 'object') {
        for (const [key, value] of Object.entries(headers)) {
            curl += ` -H '${escapeForShell(key)}: ${escapeForShell(value)}'`;
        }
    }
    
    if (body && bodyFormat === 'json' && ['POST', 'PUT', 'PATCH', 'DELETE'].includes(method)) {
        let bodyStr;
        ...
        if (CURL_CONFIG.maxBodyLength > 0 && bodyStr.length > CURL_CONFIG.maxBodyLength) {
            bodyStr = bodyStr.substring(0, CURL_CONFIG.maxBodyLength) + '...[TRUNCATED]';
        }
        
        curl += ` --data '${escapeForShell(bodyStr)}'`;
    }
...
const Utils = {
    ...
    sortBottomUp: function(nodes) {
        return [...nodes].sort((a, b) => {
            // 1. Primary sort: by depth (deeper nodes first - bottom to top)
            if (a.path.length !== b.path.length) {
                return b.path.length - a.path.length;
            }
            
            // 2. Secondary sort: by insertion order (preserves JSON field order)
            if (a.insertionOrder !== undefined && b.insertionOrder !== undefined) {
                return a.insertionOrder - b.insertionOrder;
            }
            
            // 3. Fallback: alphabetical sort for compatibility with non-JSON formats
    ...
    sortTopDown: function(nodes) {
        return [...nodes].sort((a, b) => {
            // 1. Primary sort: by depth (shallower nodes first - top to bottom)
            if (a.path.length !== b.path.length) {
...
const MutationLogger = {
    ...
    logReplace: function(pathStr, oldValue, newValue, isEnabled) {
        ...
        console.info(`  [REPLACE] ${pathStr}: ${oldStr} → ${newStr}`);
    },
    logDelete: function(pathStr, deletedValue, isEnabled) {
        ...
        if (typeLabel === 'object' || typeLabel === 'array') {
            console.info(`  [DELETE]  ${pathStr} [${typeLabel}]`);
        } else {
            console.info(`  [DELETE]  ${pathStr}: ${valuePreview} [${typeLabel}]`);
        }
    },
...
const JSONStructureMap = {
    printMap: function(template, provider, allNodes, bodyFormat, TemplateProcessor, Utils) {
        ...
        console.info('📦 JSON STRUCTURE MAP:');
        ...
        for (const node of primitiveNodes) {
            ...
            console.info(`${node.pathStr} = ${formattedValue}`);
        }
    }
};
const checkFailFast = function(stats, activeConfig) {
    ...
    const windowSize = activeConfig.failFast.windowSize || 20; // fallback for compatibility
    const recentResults = getAllRecentResults(stats, windowSize);
    ...
    if (serviceErrors >= activeConfig.failFast.maxServiceErrors) {
        ...
        stats.abortReasons.push(
            `Fail-fast triggered: ${serviceErrors} service errors (5xx) in last ${windowSize} requests`
        );
        return true;
    }
    ...
    if (rateLimitErrors >= activeConfig.failFast.maxRateLimitErrors) {
        stats.abortReasons = stats.abortReasons || [];
        stats.abortReasons.push(
            `Fail-fast triggered: ${rateLimitErrors} rate limit errors (429) in last ${windowSize} requests`
        );
        return true;
    }
...
const createJSONProvider = function() {
    return {
        name: 'JSON',
        buildInsertionOrderMap: function(obj, parentPath, orderMap, counter) {
            ...
            if (Array.isArray(obj)) {
                for (let i = 0; i < obj.length; i++) {
                    const currentPath = parentPath.concat(i);
                    const pathKey = currentPath.join('.');
                    orderMap[pathKey] = counter.value++;
                    this.buildInsertionOrderMap(obj[i], currentPath, orderMap, counter);
                }
            } else {
                // IMPORTANT: Object.keys() preserves insertion order (ES2015+)
                ...
                for (let i = 0; i < keys.length; i++) {
                    ...
                    const pathKey = currentPath.join('.');
                    orderMap[pathKey] = counter.value++;
                    this.buildInsertionOrderMap(obj[key], currentPath, orderMap, counter);
                }
            }
            
            return orderMap;
        },
        collectNodes: function(obj, path, pathStr, depth, visited, MEMORY_LIMITS, insertionOrderMap) {
            path = path || [];
            pathStr = pathStr || '';
            depth = depth || 0;
            ...
            // Build insertion order map on first call (depth === 0)
            if (depth === 0 && !insertionOrderMap) {
                insertionOrderMap = this.buildInsertionOrderMap(obj);
            }
            ...
            if (obj === null || typeof obj !== 'object') {
                const insertionOrder = insertionOrderMap && pathStr ? insertionOrderMap[pathStr] : undefined;
                nodes.push({
                    path: path.slice(),
            ...
            if (Array.isArray(obj)) {
                ...
                for (let i = 0; i < obj.length; i++) {
                    ...
                    const childNodes = this.collectNodes(
                        ...
                        depth + 1, visited, MEMORY_LIMITS, insertionOrderMap
                    );
            ...
            while (attempt < maxAttempts) {
                try {
                        ...
                        const requestPromise = new Promise((requestResolve, requestReject) => {
                            pm.sendRequest(requestConfig, (err, res) => {
                                ...
                                requestResolve({
                                    ...
                                    // Request snapshot for curl generation (only for failed requests)
                                    requestSnapshot: (code >= 300 || code < 200) ? {
                                        method: method.toUpperCase(),
                                        url: requestConfig.url || url,  // Use modified URL (with query params if present)
                                        headers: JSON.parse(JSON.stringify(requestConfig.header || {})),
                                        body: bodyFormat === 'json' ? (requestConfig.body?.raw || bodyData) : bodyData,
                    ...
                    if (shouldRetryError) {
                        ...
                        console.info(`⚠️ Retry ${attempt + 1}/${maxAttempts - 1} (delay: ${delay}ms)`);
        ...
        try {
            ...
            if (ENABLE_JSON_COMMENTS) {
                ...
                if (ENABLE_MUTATION_LOGGING && processedContent !== rawContent) {
                    ...
                    console.info(`ℹ️  JSON comments processed: ${charDiff} characters stripped`);
                }
            } else {
...
const MutationStages = {
    async executeBaseline(method, url, templateOriginal, bodyFormat, activeConfig, Utils, TemplateProcessor, httpClient, rateLimiter, stats, ResponseAnalyzer) {
        ...
        console.info('🎯 STAGE 1: BASELINE');
        
        ResponseAnalyzer.resetIdenticalErrorCounter();
        
        const responses = [];
        const repeatCount = STAGE_CONFIG.baseline.repeatCount;
        for (let i = 0; i < repeatCount; i++) {
            ...
            const result = await httpClient.send(
                method, url, processed, `BASELINE ${i + 1}/${repeatCount}`, bodyFormat, rateLimiter
            );
            if (result) {
                ...
                if (ENABLE_MUTATION_LOGGING) {
                    console.info(`  ${i + 1}/${repeatCount}: [${result.code}] ${result.responseTime || 0}ms`);
                }
            }
            if (i < repeatCount - 1) await Utils.delay(activeConfig.delayMs);
        }
    ...
    async executeFuzzing(method, url, templateOriginal, allNodes, allStagesResults, bodyFormat, provider, activeConfig, Utils, TemplateProcessor, httpClient, rateLimiter, stats, ResponseAnalyzer, PostmanAdapter) {
        ...
        if (activeConfig.targetPathsKeys && activeConfig.targetPathsKeys.length > 0 && bodyFormat !== 'json') {
            console.info(`ℹ️  Note: targetPathsKeys filtering is NOT ACTIVE for ${bodyFormat} format (JSON only)`);
        }
        ...
        if ((bodyFormat === 'queryparams' || bodyFormat === 'urlencoded') && 
            ...
            if (ENABLE_MUTATION_LOGGING) {
                console.info(`ℹ️  Auto-switched to testValues for ${bodyFormat} format (${activeConfig.mutationPolicies.testValues.length} specialized values)`);
                console.info(`   (Original policies: ${activeConfig.fuzzing.usePolicies.join(', ')})\n`);
            }
        ...
        console.info(`🔀 STAGE 2: FUZZING - Primitives: ${primitiveNodes.length} | Objects: ${objectNodes.length} | Arrays: ${arrayNodes.length}`);
        for (const node of primitiveNodes) {
            if (checkGlobalTimeout(stats, activeConfig) || checkFailFast(stats, activeConfig)) {
                console.info('🛑 Primitive fuzzing aborted');
                return;
            }
        ...
        if (activeConfig.fuzzing.fuzzObjects) {
            if (bodyFormat === 'formdata' || bodyFormat === 'urlencoded') {
                if (ENABLE_MUTATION_LOGGING) {
                    console.info(`⏭️  Object fuzzing skipped: ${bodyFormat} format doesn't support objects\n`);
                }
                ...
                const maxObjects = activeConfig.fuzzing.maxObjectsToFuzz ?? 20;
                const limitedObjects = maxObjects === 0 ? objectNodes : objectNodes.slice(0, maxObjects);
                
                if (ENABLE_MUTATION_LOGGING && limitedObjects.length < objectNodes.length) {
                    console.info(`📦 Fuzzing ${limitedObjects.length} of ${objectNodes.length} objects (limit: ${maxObjects})`);
                } else {
                    console.info(`📦 Fuzzing ${limitedObjects.length} objects`);
                }
                
                const objectMutations = activeConfig.mutationPolicies.objectMutations || [{}, null, {"key": "value"}];
                for (const node of limitedObjects) {
        ...
        if (activeConfig.fuzzing.fuzzArrays) {
            if (bodyFormat === 'formdata' || bodyFormat === 'urlencoded') {
                if (ENABLE_MUTATION_LOGGING) {
                    console.info(`⏭️  Array fuzzing skipped: ${bodyFormat} format doesn't support arrays\n`);
                }
                ...
                const maxArrays = activeConfig.fuzzing.maxArraysToFuzz ?? 20;
                const limitedArrays = maxArrays === 0 ? arrayNodes : arrayNodes.slice(0, maxArrays);
                
                if (ENABLE_MUTATION_LOGGING && limitedArrays.length < arrayNodes.length) {
                    console.info(`📊 Fuzzing ${limitedArrays.length} of ${arrayNodes.length} arrays (limit: ${maxArrays})`);
                } else {
                    console.info(`📊 Fuzzing ${limitedArrays.length} arrays`);
                }
                
                const arrayMutations = activeConfig.mutationPolicies.arrayMutations || [[], null, [[]], [null]];
                for (const node of limitedArrays) {
                    if (checkGlobalTimeout(stats, activeConfig) || checkFailFast(stats, activeConfig)) return;
        ...
        if (queryKeys.length > 0) {
            console.info(`🔗 Fuzzing ${queryKeys.length} query parameters`);
            for (const key of queryKeys) {
                if (checkGlobalTimeout(stats, activeConfig) || checkFailFast(stats, activeConfig)) return;
                
                const maxMutations = activeConfig.fuzzing.maxMutationsPerParam ?? 10;
                let paramMutations = maxMutations === 0 ? uniqueMutations : uniqueMutations.slice(0, maxMutations);
                
                if (ENABLE_MUTATION_LOGGING && paramMutations.length < uniqueMutations.length) {
                    console.info(`   ℹ️  Limited to ${maxMutations} mutations for query.${key} (from ${uniqueMutations.length} unique values)`);
                }
    ...
    async executeSingleDelete(method, url, templateOriginal, allNodes, allStagesResults, bodyFormat, provider, activeConfig, Utils, TemplateProcessor, httpClient, rateLimiter, stats, ResponseAnalyzer) {
        ...
        if (activeConfig.targetPathsKeys && activeConfig.targetPathsKeys.length > 0) {
            console.info('ℹ️  Single Delete stage SKIPPED: targetPathsKeys is only supported for FUZZING stage');
            console.info('ℹ️  To run this stage, remove targetPathsKeys or set it to empty array []');
            return;
        }
        ...
        console.info(`🗑️ STAGE 3: SINGLE DELETE - Testing ${deleteNodes.length} deletions`);
            ...
            if (provider && provider.deleteByPath(processed, node.path)) {
                ...
                if (provider && provider.cleanupEmpty && bodyFormat === 'json') {
                    ...
                    if (beforeSize - afterSize > 50) {
                        if (ENABLE_MUTATION_LOGGING) {
                            console.info(`   🧹 Cleaned up empty containers: ${beforeSize} → ${afterSize} bytes`);
                        }
                    }
                }
                if (typeof PostmanAdapter !== 'undefined') {
    ...
    async executeCombinatorialDelete(method, url, templateOriginal, allNodes, allStagesResults, bodyFormat, provider, activeConfig, Utils, TemplateProcessor, httpClient, rateLimiter, stats, ResponseAnalyzer) {
        ...
        if (activeConfig.targetPathsKeys && activeConfig.targetPathsKeys.length > 0) {
            console.info('ℹ️  Combinatorial Delete stage SKIPPED: targetPathsKeys is only supported for FUZZING stage');
            console.info('ℹ️  To run this stage, remove targetPathsKeys or set it to empty array []');
            return;
        }
        ...
        console.info(`🧩 STAGE 4: COMBINATORIAL DELETE (Pairwise Testing) - Generated ${combinations.length} combinations`);
        ...
        for (const combo of combinations) {
            ...
            const deletedNodes = [];
            if (ENABLE_MUTATION_LOGGING) {
                console.info(`  [COMBO-DELETE] Combination of ${combo.length} fields:`);
            }
    ...
    async executeProgressiveDelete(method, url, templateOriginal, allNodes, allStagesResults, bodyFormat, provider, activeConfig, Utils, TemplateProcessor, httpClient, rateLimiter, stats, ResponseAnalyzer) {
        ...
        if (activeConfig.targetPathsKeys && activeConfig.targetPathsKeys.length > 0) {
            console.info('ℹ️  Progressive Delete stage SKIPPED: targetPathsKeys is only supported for FUZZING stage');
            console.info('ℹ️  To run this stage, remove targetPathsKeys or set it to empty array []');
            return;
        }
        ...
        console.info(`📉 STAGE 5: PROGRESSIVE DELETE (Top-Down Greedy) - Starting with ${sortedNodes.length} nodes`);
        
        if (sortedNodes.length > 0) {
            const minDepth = Math.min(...sortedNodes.map(n => n.path.length));
            const maxDepth = Math.max(...sortedNodes.map(n => n.path.length));
            if (ENABLE_MUTATION_LOGGING) {
                console.info(`ℹ️  Strategy: Deleting from root to leaves (depth ${minDepth} → ${maxDepth})\n`);
            }
        }
        ...
        for (let i = 0; i < sortedNodes.length; i++) {
            if (checkGlobalTimeout(stats, activeConfig) || checkFailFast(stats, activeConfig)) {
                console.info(`⏹️ Progressive Delete stopped at field ${i + 1}/${sortedNodes.length}`);
                break;
            }
            ...
            if (result) {
                ...
                if (result.code >= 200 && result.code < 300) {
                    ...
                    if (!cfg.continueOnError) {
                        console.info(`⏹️ Stopping at first required field (continueOnError=false)`);
                        break;
                    }
                }
            }
            if (result?.aborted || ResponseAnalyzer.checkAbort(result, activeConfig, stats)) {
                console.info(`⏹️ Progressive Delete aborted`);
                break;
            }
            await Utils.delay(activeConfig.delayMs);
        }
        if (bodyFormat === 'json') {
            ...
            if (hasContent && stats.requiredFields.size === 0) {
                ...
                if (result) {
                    ...
                    safeAddResult(stats.stageResults.progressiveDelete, result, HARD_RESULTS_CAP);
                    if (result.code >= 200 && result.code < 300) {
                        console.info('   ✅ Empty payload accepted - API requires NOTHING!');
                        currentPayload = emptyPayload;
                        stats.acceptsEmptyPayload = true;
                    } else {
                        console.info(`   ❌ Empty payload rejected (${result.code})`);
                        console.info(`   ⚠️  Note: 0 required fields detected, but API rejects empty payload`);
                        console.info(`   ⚠️  This may indicate required containers or business logic constraints`);
        ...
        const sizeThreshold = cfg.minimalPayloadSizeThreshold ?? 1000;
        if (bodyFormat === 'json' && cfg.logMinimalPayload && (sizeThreshold === 0 || finalSize < sizeThreshold)) {
            console.info(`📋 Minimal working payload (${finalSize} bytes):`);
            console.info(JSON.stringify(currentPayload, null, 2));
        }
    },
    ...
    updateStatsFromResult(result, stats, ResponseAnalyzer, activeConfig, Utils) {
        ...
        if (MAX_TOTAL_REQUESTS > 0 && stats.total > MAX_TOTAL_REQUESTS) {
            ...
            console.error('╚═══════════════════════════════════════════════════════════╝');
            console.error(`Request quota limit reached: ${MAX_TOTAL_REQUESTS}`);
            console.error(`Current requests: ${stats.total}`);
            console.error(`💡 This protects against API quota exhaustion`);
        ...
        if (activeConfig.logShowResponse) {
            ...
            console.info(`${icon} [${result.code}] ${result.label} (${result.responseTime || 0}ms)`);
        }
    }
};
const BugClustering = {
    ...
    clusterBySignature: function(results) {
        ...
        const filteredResults = results.filter(result => {
            const statusCode = parseInt(result.statusCode || result.code || 0);
            
            if (CLUSTER_CONFIG.excludeSuccessCodes && statusCode >= 200 && statusCode < 300) {
                return false;
            }

            if (statusCode >= 500 && statusCode < 600) {
                return true;  // Auto-include all server errors
            }
            
            if (CLUSTER_CONFIG.includeCodes && CLUSTER_CONFIG.includeCodes.length > 0) {
                return CLUSTER_CONFIG.includeCodes.includes(statusCode);
            }
        ...
        for (const result of filteredResults) {
            ...
            const rawMessage = 
                ...
                (result.responseBody && typeof result.responseBody === 'object' 
                    ? (result.responseBody.error || 
                       ...
                       result.responseBody.details ||
                       result.responseBody.description ||
                       JSON.stringify(result.responseBody).substring(0, 200))
            ...
            if (!clusters[clusterKey]) {
                clusters[clusterKey] = {
                    ...
                    count: 0,
                    sampleOccurrences: [],
                    exampleCurl: null  // Curl command for reproducing this bug
                };
            }
            ...
            if (clusters[clusterKey].sampleOccurrences.length < this.MAX_OCCURRENCES_SAMPLES) {
                ...
                let mutationDetails = null;
                
                if (result.requestSnapshot && CLUSTER_CONFIG.detailedCurlOutput) {
                    curlCommand = buildCurl(result.requestSnapshot);
                }
                
                if (CLUSTER_CONFIG.showMutationDetails) {
                    mutationDetails = {
                        mutationPath: result.mutationPath || null,
                        mutationType: result.mutationType || null,
                        deletedFields: result.deletedFields || null
                    };
                }
            ...
            if (!CLUSTER_CONFIG.detailedCurlOutput && !clusters[clusterKey].exampleCurl && result.requestSnapshot) {
                clusters[clusterKey].exampleCurl = buildCurl(result.requestSnapshot);
                clusters[clusterKey].exampleCurlLabel = result.label;
            }
        }
        
        const filteredClusters = Object.values(clusters).filter(
            cluster => cluster.count >= CLUSTER_CONFIG.minOccurrencesToShow
        );
        
        const sortedClusters = filteredClusters.sort((a, b) => b.count - a.count);
...
const validateRequiredVariables = function(templateOriginal, bodyFormat, activeConfig, TemplateProcessor) {
    ...
    if (missingVars.length > 0) {
        ...
        missingVars.forEach(v => {
            console.info(`   pm.collectionVariables.set('${v}', <value>);`);
        });
...
const printSummary = function(stats, ENGINE_VERSION, activeConfig, BugClustering, PostmanAdapter) {
    ...
    console.info('────────────────────────────────────');
    console.info(`Requests:   ${stats.total} | Skipped: ${stats.skipped}`);
    console.info(`Duration:   ${duration}s | Avg: ${avgResp}ms`);
    console.info(`Format:     ${stats.bodyFormat.toUpperCase()}`);
    if (stats.abortReasons.length > 0) {
        console.info('🛑 ABORTED');
        console.info('────────────────────────────────────');
        stats.abortReasons.forEach((r, i) => console.info(`${i + 1}. ${r}`));
    }
    console.info('📢 HTTP CODES');
    console.info('────────────────────────────────────');
    if (stats.total > 0) {
        ...
        for (const [code, count] of sorted) {
            ...
            console.info(`${emoji} [${String(code).padEnd(7)}]: ${String(count).padStart(4)} (${pct}%)`);
        }
    }
    if (stats.bugClusters && stats.bugClusters.length > 0) {
        const sortedClusters = BugClustering.getSortedClusters(stats.bugClusters);
        
        console.info(`🧬 BUG CLUSTERS - total ${sortedClusters.length}`);
        console.info('────────────────────────────────────');
        
        const maxClusters = CLUSTER_CONFIG.maxClustersToShow || 40;
        
        sortedClusters.slice(0, maxClusters).forEach((cluster, idx) => {
            const code = cluster.code || cluster.statusCode || 'unknown';
            const errorClass = cluster.errorClass || cluster.signature || 'unknown_error';
            console.info(`${idx + 1}. [${code}] ${errorClass} - ${cluster.count}× occurrences`);
            
            if (CLUSTER_CONFIG.detailedCurlOutput && cluster.sampleOccurrences && cluster.sampleOccurrences.length > 0) {
                const maxExamples = Math.min(
                    CLUSTER_CONFIG.maxCurlsPerCluster || 3,
                    cluster.sampleOccurrences.length
                );
                const examples = cluster.sampleOccurrences.slice(0, maxExamples);
                
                
                examples.forEach((sample, idx) => {
                    const label = sample.label || 'unknown';
                    const exampleNum = idx + 1;
                    const totalExamples = examples.length;
                    
                    console.info(`   Example ${exampleNum}/${totalExamples}: ${label}`);
                    
                    if (CLUSTER_CONFIG.showMutationDetails && sample.mutationDetails) {
                        const details = sample.mutationDetails;
                        if (details.mutationType) {
                            console.info(`   Mutation type: ${details.mutationType}`);
                        }
                        if (details.deletedFields && details.deletedFields.length > 0) {
                            console.info(`   Deleted fields: ${details.deletedFields.join(', ')}`);
                        }
                    }
                    
                    if (sample.curl) {
                        console.info('   ╭─ 📋 Copy this curl command:');
                        const curlCommand = sample.curl
                            .replace(/\r?\n|\r/g, '')
                            .trim();
                        ...
                        if (typeof process !== 'undefined' && 
                            ...
                            process.stdout.write('   ' + curlCommand + '\n');
                        } else {
                            console.info('   ' + curlCommand);
                        }
                        console.info('   ╰─────────────────────────────────────────────');
                    } else {
                        console.info('   ⚠️  No curl available for this example');
                    }
                    
                });
                
                if (cluster.sampleOccurrences.length > maxExamples) {
                    const remaining = cluster.sampleOccurrences.length - maxExamples;
                    console.info(`   ... +${remaining} more examples (set CLUSTER_CONFIG.maxCurlsPerCluster to see more)`);
                }
                
            } else if (!CLUSTER_CONFIG.detailedCurlOutput && cluster.exampleCurl) {
                
                if (cluster.exampleCurlLabel) {
                    console.info(`   ╭─ 📋 Copy this curl command for: ${cluster.exampleCurlLabel}`);
                } else {
                    console.info('   ╭─ 📋 Copy this curl command (first occurrence):');
                }
                console.info('   │');
                
                const curlCommand = cluster.exampleCurl
                    .replace(/\r?\n|\r/g, '')
                    .trim();
                ...
                if (cluster.sampleOccurrences && cluster.sampleOccurrences.length > 0) {
                    const maxExamples = Math.min(3, cluster.sampleOccurrences.length);
                    const examples = cluster.sampleOccurrences.slice(0, maxExamples);
                    
                    examples.forEach(sample => {
                        const label = sample.label || 'unknown';
                        const isActiveCurl = cluster.exampleCurlLabel && label === cluster.exampleCurlLabel;
                        const marker = isActiveCurl ? '👉' : '   ';
                        const annotation = isActiveCurl ? ' ← curl above' : '';
                        console.info(`${marker} ${label}${annotation}`);
                    });
                    
                    if (cluster.sampleOccurrences.length > maxExamples) {
                        const remaining = cluster.sampleOccurrences.length - maxExamples;
                        console.info(`   └─ ... +${remaining} more examples`);
                    }
                }
            }
        });
        
        if (sortedClusters.length > maxClusters) {
            console.info(`  ... +${sortedClusters.length - maxClusters} more clusters`);
        }
    }
    if (stats.requiredFields.size > 0) {
        console.info('🔒 REQUIRED FIELDS');
        console.info('────────────────────────────────────');
        const fields = Array.from(stats.requiredFields).slice(0, 10);
        fields.forEach(f => console.info(`  - ${f}`));
        if (stats.requiredFields.size > 10) {
            console.info(`  ... +${stats.requiredFields.size - 10} more`);
        }
    }
    if (activeConfig.features.postmanTest) {
        const totalBugs = stats.failureSignatures.size || (stats.bugClusters ? stats.bugClusters.length : 0);
        if (totalBugs > 0) {
...
(async function main() {
    ...
    if (GLOBAL_TIMEOUT_MS > 0) {
        globalTimeoutHandle = setTimeout(() => {
            ...
            console.error('╚═══════════════════════════════════════════════════════════╝');
            console.error(`Maximum execution time (${GLOBAL_TIMEOUT_MS}ms) exceeded`);
            console.error(`Elapsed: ${elapsed}s`);
            console.error(`💡 This prevents infinite hangs in Newman/CI environments`);
    ...
    try {
        ...
        if (bodyFormat === 'json') {
            ...
            if (MEMORY_LIMITS.maxCollectNodesSize > 0 && allNodes.length > MEMORY_LIMITS.maxCollectNodesSize) {
                ...
                console.error('╚═══════════════════════════════════════════════════════════╝');
                console.error(`JSON contains too many nodes: ${allNodes.length}`);
        ...
        if (activeConfig.targetPathsKeys && activeConfig.targetPathsKeys.length > 0) {
            ...
            if (filteredNodes.length > 0) {
                console.info(`ℹ️  Path filtering active: testing ${filteredNodes.length} of ${allNodes.length} nodes`);
                console.info(`ℹ️  Note: Only FUZZING stage is supported with targetPathsKeys`);
            }
        }
